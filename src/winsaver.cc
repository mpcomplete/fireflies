...
void read_config() {
  ...
  scene.set_defaults();
  if (RegOpenKeyEx(HKEY_CURRENT_USER, MY_HKEY,
                   0,  // reserved
...
void write_config(HWND hDlg) {
  ...
  scene.wind_speed = ((int)GetDlgItemInt(hDlg, IDC_CONF_WIND, 0, TRUE)) / 10.0;
  scene.draw_bait =
      (IsDlgButtonChecked(hDlg, IDC_CONF_DRAWBAIT) == BST_CHECKED);
  scene.fast_forward = (int)GetDlgItemInt(hDlg, IDC_CONF_FASTFORWARD, 0, TRUE);
  fps = (int)GetDlgItemInt(hDlg, IDC_CONF_FPS, 0, TRUE);
  for (GLuint i = 0; i < NUM_BMODES; i++) {
    scene.bmodes.change(
        i, (double)(UINT)GetDlgItemInt(hDlg, IDC_CONF_BMODE(i), 0, FALSE));
  }
  for (GLuint i = 0; i < NUM_SMODES; i++) {
    scene.smodes.change(
        i, (double)(UINT)GetDlgItemInt(hDlg, IDC_CONF_SMODE(i), 0, FALSE));
  }

  if (RegCreateKeyEx(
          HKEY_CURRENT_USER, MY_HKEY,
          0,   // reserved
...
void set_dialog(HWND hDlg) {
  ...
  SetDlgItemInt(hDlg, IDC_CONF_TAILLENGTH, (UINT)(scene.tail_length * 10),
                TRUE);
  ...
  SetDlgItemInt(hDlg, IDC_CONF_GLOWFACTOR, (UINT)(scene.glow_factor * 10),
                TRUE);
