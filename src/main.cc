...
int argp_parse(struct argp* argp_s, int argc, char* argv[], int, int*, void*) {
  ...
  for (int i = 1; i < state.argc; i = state.next) {
    ...
    if (optind < 0) {  // doesn't exist
      cerr << state.name << ": unrecognized option `" << state.argv[i] << "'"
           << endl;
      cerr << "Try `" << state.name << " --help' or `" << state.name
           << " --usage' for more information" << endl;
      return -1;
    }
    if (argp_s->options[optind].arg != 0 && arg == 0) {
      state.next = i + 2;
      if (i + 1 >= state.argc) {
        cerr << state.name << ": option requires an argument -- " << opt
             << endl;
        return -1;
      }
      arg = state.argv[i + 1];
...
const char* const mode_help =
    ...
    "Note: mode probabilities are relative. For example, if A has p=5, and B "
    "has\n"
...
static char doc[] = "Moving swarms of delicious eye candy.";
...
static struct argp_option options[] = {
    ...
    {"fastforward", OPT_FASTFORWARD, "NUM", 0,
     "Fast forward factor (default = 1)"},
  ...
  switch (key) {
    ...
    case OPT_FASTFORWARD:
      ...
      if (scene.fast_forward == 0) {
        cerr << state->name << ": -fastforward must be > 0" << endl;
        return -1;
      }
      break;
    case 'b':
    ...
    case 'o':
      ...
      if (scene.tail_opaq < 0 || scene.tail_opaq > 1) {
        cerr << state->name << ": -tailopacity must be in the range [0,100]"
             << endl;
        return -1;
      }
      break;
    ...
    case 'm': {
      ...
      if (state->next < state->argc) {
        ...
        cerr << state->name << ": option -modebait requires 2 arguments"
             << endl;
        return -1;
      }
