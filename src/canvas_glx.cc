...
Window make_window(Display* display)
{
  if (!display)
  {
  ...
  Window win = XCreateWindow( display, RootWindow( display, vi->screen ), 
                              0, 0, 1024, 1024, 0, vi->depth, InputOutput, 
                              vi->visual, 
...
int CanvasGLX::handle_events()
{
  ...
  while (XPending(display)) {
    ...
    case KeyPress: {
      ...
      if (isalpha(sym))
        cout << "KeyPress: " << event.xkey.keycode << " = "
          << XKeycodeToKeysym(display, event.xkey.keycode, 0) << endl;
